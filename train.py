...
def validate_bboxes(bboxes, words, ann_path):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if len(bbox) != 4:
            print(f"⚠️  [{ann_path}] bbox index {i} (word='{word}') "
...
class GSTInvoiceDataset(Dataset):
    ...
    def __init__(self, image_dir, annotation_dir, processor, max_length=512):
        ...
        for ann_path in sorted(self.annotation_dir.glob("*.json")):
            ...
            if img_path is None:
                try:
                    ...
                    if json_img and Path(json_img).exists():
                        ...
                        print(f"⚠️  Skipping {ann_path.name} — image not found.")
                        skipped += 1
                        continue
                except Exception:
                    print(f"⚠️  Skipping {ann_path.name} — could not read JSON.")
                    skipped += 1
                    continue
            ...
            try:
                ...
                if "layout" not in tmp or "words" not in tmp["layout"]:
                    print(f"⚠️  Skipping {ann_path.name} — missing layout.words.")
                    skipped += 1
                    continue
                ...
                if not words_data:
                    print(f"⚠️  Skipping {ann_path.name} — no valid words.")
                    skipped += 1
                    continue
                ...
                if not valid_words:
                    print(f"⚠️  Skipping {ann_path.name} — no words left after bbox filter.")
                    skipped += 1
                    continue

            except Exception as e:
                print(f"⚠️  Skipping {ann_path.name} — error during pre-validation: {e}")
                skipped += 1
                continue
        ...
        if skipped:
            print(f"⚠️  Skipped : {skipped} samples (bad or empty)")
...
def verify_dataset(ann_dir, img_dir):
    ...
    for ann_file in ann_files:
        ...
        if len(matches) == 0:
            ...
            issues.append(f"⚠️  DUPLICATE images for {ann_file}: {matches}")
