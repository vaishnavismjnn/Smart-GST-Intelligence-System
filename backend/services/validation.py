...
def validate_amounts(total, taxable, gst):
    ...
    Tolerance raised to 10 to absorb:
      - rounding across multiple line items
      - OCR digit misreads on paise values
      - explicit Round Off rows on invoices (common ±5)
    Returns True when any value is missing (nothing to validate).
    """
