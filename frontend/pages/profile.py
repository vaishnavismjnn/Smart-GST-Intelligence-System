...
from utils.api import login, get_records
...
def show():
    ...
    with col_left:
        # Avatar card with animated ring
        st.markdown(f"""
        ...
        valid_cnt = sum(1 for r in processed if r.get("validation", {}).get("gst_valid"))
        ...
        # API status
        from utils.api import health_check, BASE_URL
        is_live = health_check()
        sc = "#00D4AA" if is_live else "#FF4D6D"
        st.markdown(f"""
    ...
    with col_right:
        ...
        <div style="background:rgba(255,77,109,0.05); border:1px solid rgba(255,77,109,0.2);
            ...
            <div style="color:#A0AEC0; font-size:0.82rem; margin-bottom:1rem; line-height:1.5;">
                Signing out clears your JWT session. You'll need to log in again to access the platform.
            </div>
        </div>
        """, unsafe_allow_html=True)
