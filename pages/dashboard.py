...
def show() -> None:
    ...
    with col_summary:
        ...
        if valid:
            ...
            all_processed_gst = sum(
                clean_amount(r.get("GST_AMOUNT")) for r in processed_deduped
            )
            pct = min(100, int((itc / all_processed_gst) * 100)) if all_processed_gst > 0 else 00
        ...
        # ================== DAWN GUARD DEMO BUTTON ==================
    
    st.markdown("### 🌅 Dawn Guard Demo")
    st.caption("Click below to generate brief from ALL invoices in database")

    if st.button("Generate Dawn Guard Brief (Demo)"):
        if not is_authenticated():
        ...
        with st.spinner("Analysing all invoices and generating brief..."):
            try:
                from dawn_guard_agent import run_dawn_guard_demo

                brief = run_dawn_guard_demo(user_email=user_email)

                st.success(f"✅ Brief sent to {user_email}")
                st.markdown(brief)
